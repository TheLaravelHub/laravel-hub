...
const Edit = ({package: singlePackage, indexes, categories}: EditProps) => {
    ...
    const { post, data, setData, processing, errors, reset, isDirty } = useForm<{
        _method: string
                                    ...
                                    <Card>
                                        <CardContent className="flex flex-col justify-center gap-4 py-3">
                                            ...
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <MultiSelect
                                                    options={categories}
                                                    value={categories.filter(option => data.category_ids?.map(String).includes(option.value.toString()))} // Use data.category_ids instead of singlePackage
                                                    onChange={(selectedOptions) => {
                                                        const selectedValues = selectedOptions.map(option => option.value.toString()); // Ensure values are strings
                                                        setData("category_ids", selectedValues); // Update Inertia form data correctly
                                                    }}
                                                />
